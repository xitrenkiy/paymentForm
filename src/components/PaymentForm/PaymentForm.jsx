...
const PaymentForm = () => {
	...
	return (
		<div className="relative w-100 mx-auto">
			<div
				className={`min-h-100 relative overflow-hidden w-100 mx-auto rounded-2xl bg-gray-200 ${isAddModalOpen || isCardModalOpen
					? "opacity-20"
					: "opacity-100"
					}`}
			>
				<div className="h-16 p-5 w-100 text-black bg-white flex items-center">
					...
					<span className="text-[10] ml-8 font-medium">Wallet</span>
				</div>
				<div className="p-5 relative min-h-40 w-100">
					<div className="w-85 rounded mx-auto text-black px-5 py-3 bg-white shadow">
						...
						<div className="flex mt-1 items-center">
							...
							/>
							<div className="text-2xl ml-1.5 font-bold">
								${activeUser?.credits}
						...
						<Link
							to="/credits"
							className="w-[100%] h-15 mt-5 text-white font-medium bg-blue-500 rounded-2xl cursor-pointer text-center flex items-center justify-center transition ease-in-out duration-200 hover:bg-blue-700"
						>
							<span className="text-2xl mr-5 mb-1">+</span> Buy
							Credits
						</Link>
					</div>
				</div>
									...
									>
										...
										<div className="flex flex-col text-[14px] ml-4 tracking-wide">
											...
											>
												{item.name}
												{item.primary ? (
													<span className="ml-2 bg-gray-200 p-1 rounded-2xl text-[10px] text-blue-500">
														Primary
													</span>
												) : null}
											</div>
											<div className="text-[11px] opacity-50 mt-1">{`**** ${item?.number.slice(
												14
											)}`}</div>
											<span className="absolute top-[50%] right-0 translate-y-[-50%] cursor-pointer hover:animate-ping">
												&gt;
											</span>
										</div>
									</div>
					...
					{cards.length > 0 &&
						...
						<div className="flex items-center gap-4 mt-5 w-full bg-green-100 rounded-xl px-5 py-2 bottom-0">
							...
							<div className="text-[14px] font-light">
								We're fully complaint with the payment card
								indrustry data security standarts
							</div>
						</div>
					) : null}
					{cards.length === 0 &&
				...
				<div className="bg-white mt-2 min-h-50 text-black px-5 py-4">
					...
					<div className="text-gray-500 mt-2 text-[14px]">
						Recharge your wallet automatically when
						<br /> the balance is running low.
					</div>
					<select
						name="dollars"
						id="dollars"
						className="border border-gray-300 shadow mt-3 rounded w-full p-2"
					>
					...
					<div className="text-[14px] mt-3 text-gray-500">
						Your balance will be auto recharged when there is less
						than 3 minutes of the call you are in your account
					</div>
				</div>
			</div>

			{isAddModalOpen && !isCardModalOpen ? (
				<AddCard
					toggleModal={() =>
						handleClickAddButton(setIsAddModalOpen, isAddModalOpen)
					}
				/>
			) : null}
			{isCardModalOpen && !isAddModalOpen ? (
				<CardDetail
					toggleModal={() =>
						handleClickAddButton(
							setIsCardModalOpen,
